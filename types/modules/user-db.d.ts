...
export type UserDb = {
  getTwitchUserByUsername: (username: string) => Promise<FirebotUser | undefined>;
  /**
   ...
   */
  updateUserMetadata: (username: string, key: string, value: any, propertyPath?: string) => Promise<void>;
  /**
   ...
   */
  getUserMetadata: (username: string, key: string, propertyPath?: string) => Promise<any>;
   ...
   */
  searchUsers: (usernameFragment: string) => Promise<FirebotUser[] | undefined>;
  /**
   * Returns the top users with the most view time.
   * @param count how many users should be returned.
   */
  getTopViewTimeUsers: (count: number) => Promise<Array<{ username: string, minutesInChannel: number }>>;
